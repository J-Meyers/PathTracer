...
// Visible for tests
[[nodiscard]] std::unordered_map<std::string, MaterialParameters>
loadMaterials(std::istream &in);

static constexpr auto tokenRe =
    ctll::fixed_string{R"(\s*((#.*)|[^ \t\n\r#]+))"};

template <typename F> void parse(std::istream &in, F &&handler) {
...
void loadObjFile(std::istream &in, ObjLoaderOpener &opener, SceneBuilder &sb) {
  ...
  std::unordered_map<std::string, MaterialParameters> materials;
